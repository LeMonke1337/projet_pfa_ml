...
glove_file = "glove.6B.300d.txt"  # Use the 300d embeddings as per your request
glove_vectors = load_glove_vectors(glove_file)

# Prepare embedding matrix
vocab_size = len(tokenizer.word_index) + 1
embedding_dim = 300  # Adjusted to match GloVe embedding dimension
...
with open("accuracy/accuracy_report_glove_lstm.txt", "w") as f:
...
def predict_category_glove_lstm(
    custom_profile, tokenizer, model, label_encoder, max_length
):
    custom_text = (
    ...
    pred_prob = model.predict(custom_padded)
...
for i, profile in enumerate(test_profiles):
    predicted_category = predict_category_glove_lstm(
