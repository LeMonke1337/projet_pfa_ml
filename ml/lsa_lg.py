...
with open("accuracy/lsa_logreg_accuracy.txt", "w") as f:
...
# Function to recommend category using Logistic Regression with LSA features
def recommend_category_with_logreg(profile, tfidf_vectorizer, lsa, logreg_classifier):
    profile_text = (
    ...
    predicted_category = logreg_classifier.predict(profile_lsa)[0]
...
for i, profile in enumerate(test_profiles):
    recommended_category = recommend_category_with_logreg(
        profile, tfidf_vectorizer, lsa, logreg_classifier
    )
