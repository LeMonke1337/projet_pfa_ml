...
model = Sequential()
model.add(
    Conv1D(filters=128, kernel_size=3, activation="relu", input_shape=(n_components, 1))
)
...
with open("accuracy/accuracy_report_lsa_cnn.txt", "w") as f:
...
def predict_category_lsa_cnn(
    ...
    custom_profile_lsa_cnn = np.expand_dims(custom_profile_lsa, axis=2)
    pred_prob = model.predict(custom_profile_lsa_cnn)
...
for i, profile in enumerate(test_profiles):
    predicted_category = predict_category_lsa_cnn(
