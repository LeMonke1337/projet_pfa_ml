...
tokenizer = BertTokenizer.from_pretrained("./tokenizer_v3")
...
model = TFBertForSequenceClassification.from_pretrained(
    "./model_v3", num_labels=num_labels
)

# Compile the model (optional if you want to evaluate without compiling)
...
with open("accuracy/accuracy_report_bert.txt", "w") as f:
...
def predict_category_bert(custom_profile, tokenizer, model, label_encoder):
    custom_text = (
...
for i, profile in enumerate(test_profiles):
    predicted_category = predict_category_bert(profile, tokenizer, model, label_encoder)
